...
class Environ:
    ...
    def __init__(self, down_lane, up_lane, left_lane, right_lane, width, height, n_veh, n_neighbor):
        ...
        self.V2V_power_dB_List = [23, 10, 5, -100]  # the power levels : [23, 15, 5, -100]
        self.sig2_dB = -114
        self.bsAntGain = 8
    ...
    def add_new_vehicles_by_number(self, n):
         #####產生目標車輛#####
        start_position = [750 / 2 / 2, 1299 / 2 / 2]
    ...
    def Compute_Performance_Reward_Train(self, actions_power, i_episode):
        ...
        V2V_Rate_threshold = 0.001 # V2V payload: 0.2M bits every 1 s
        A = 10
        ...
        return V2I_Rate, V2V_Rate, reward_elements, F_x
    ...
    def Compute_Performance_Reward_Test_rand(self, actions_power):
        ...
        V2I_Signals = 10 ** ((self.V2I_power_dB - self.V2I_channels_with_fastfading.diagonal() + self.vehAntGain + self.bsAntGain - self.bsNoiseFigure) / 10)
        V2I_Rate = (self.bandwidth / (1e6)) * np.log2(1 + np.divide(V2I_Signals, self.V2I_Interference_random))
    ...
    def Compute_Performance_Reward_Test_sarl(self, actions_power):
        ...
        V2I_Signals = 10 ** ((self.V2I_power_dB - self.V2I_channels_with_fastfading.diagonal() + self.vehAntGain + self.bsAntGain - self.bsNoiseFigure) / 10)
        V2I_Rate = (self.bandwidth / (1e6)) * np.log2(1 + np.divide(V2I_Signals, V2I_Interference_sarl))
        ...
        V2V_Rate = (self.bandwidth / (1e6)) * np.log2(1 + np.divide(V2V_Signal, V2V_Interference_sarl))
    ...
    def Compute_Performance_Reward_Test_dpra(self, actions_power):
        ...
        V2I_Signals = 10 ** ((self.V2I_power_dB - self.V2I_channels_with_fastfading.diagonal() + self.vehAntGain + self.bsAntGain - self.bsNoiseFigure) / 10)
        V2I_Rate = (self.bandwidth / (1e6)) * np.log2(1 + np.divide(V2I_Signals, self.V2I_Interference_dpra))
        ...
        V2V_Rate = (self.bandwidth / (1e6)) * np.log2(1 + np.divide(V2V_Signal, self.V2V_Interference_dpra))
    ...
    def act_for_training(self, actions, i_episode):
        ...
        V2I_Rate, V2V_Rate, reward_elements, F_x = self.Compute_Performance_Reward_Train(action_temp, i_episode)
        ...
        # reward = lambdda * np.sum(V2I_Rate) + (1 - lambdda) * np.sum(F_x)
        reward = lambdda * np.sum(V2I_Rate) + (1 - lambdda) * F_x

        return reward

    def act_for_testing(self, actions, i_episode):

        action_temp = actions.copy()
        V2I_Rate, V2V_Rate, reward_elements, F_x = self.Compute_Performance_Reward_Train(action_temp, i_episode)
        count = np.zeros(shape=1, dtype='int32')
        for i in range(len(V2V_Rate)):
            if V2V_Rate[i] >= self.V2V_Rate_threshold:
                count += 1
            else:
                continue
