...
# def Agent(self, memory_entry_size):
#     self.discount = 1
#     self.double_q = True
#     self.memory_entry_size = memory_entry_size
#     self.memory = ReplayMemory(self.memory_entry_size)
...
target_update_step = n_step_per_episode*4    # target_update_step = n_step_per_episode*4
...
def q_learning_mini_batch(current_agent, current_sess):
    ...
    # print("batch_reward大小:", len(batch_reward))
...
record_loss = []
vehicle_position = np.zeros([int(n_episode/100), n_veh, 2])
V2V_action_selection_train = np.zeros([n_episode, n_step_per_episode, n_veh, n_neighbor, 2])
...
if IS_TRAIN:
        ...
        if i_episode%100 == 0:
            ...
            for i in range(n_veh):
                # print("vehicle_position:", env.vehicles[i].position)
                   vehicle_position[int(i_episode/100), i, :] = env.vehicles[i].position
        ...
        for i_step in range(n_step_per_episode):
            ...
            for i in range(n_veh):
                for j in range(n_neighbor):
                    ...
                    V2V_action_selection_train[i_episode, i_step, i, j, 0] = action_all_training[i, j, 0]
                    V2V_action_selection_train[i_episode, i_step, i, j, 1] = action_all_training[i, j, 1]
    ...
    # model_path = label + '/agent_' + str(0)
    # save_models(sesses[0], model_path)
    # model_path = label + '/agent_' + str(0)
    # print('Saving model: ', 0)
    # save_models(sesses[0], model_path)
    # model_path = label + '/agent_' + str(1)
    # print('Saving model: ', 1)
    # save_models(sesses[1], model_path)
    # model_path = label + '/agent_' + str(2)
    # print('Saving model: ', 2)
    # save_models(sesses[2], model_path)
    # model_path = label + '/agent_' + str(3)
    # print('Saving model: ', 3)
    # save_models(sesses[3], model_path)
    # model_path = label + '/agent_' + str(4)
    # print('Saving model: ', 4)
    # save_models(sesses[4], model_path)
    # model_path = label + '/agent_' + str(5)
    # print('Saving model: ', 5)
    # save_models(sesses[5], model_path)
    # saver.save(sesses[0], model_path, write_meta_graph=False)
    for i in range(n_veh):
        for j in range(n_neighbor):
            model_path = label + '/agent_' + str(i * n_neighbor + j)
            print('Saving model: ', i * n_neighbor + j)
    ...
    current_dir = os.path.dirname(os.path.realpath(__file__))
    marl_path = os.path.join(current_dir, "model/" + label + '/vehicle_position.mat')
    scipy.io.savemat(marl_path, {'vehicle_position': vehicle_position})
    
    marl_path = os.path.join(current_dir, "model/" + label + '/V2V_action_selection_train.mat')
    scipy.io.savemat(marl_path, {'V2V_action_selection_train': V2V_action_selection_train})
