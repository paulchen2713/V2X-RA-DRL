...
class V2Vchannels:
    ...
    def get_path_loss(self, position_A, position_B):
        ...
        d = math.hypot(d1, d2) + 0.001                         # math.hypot(x,y) : sqrt(x*x + y*y)
...
class Environ:
    ...
    def __init__(self, down_lane, up_lane, left_lane, right_lane, width, height, n_veh, n_neighbor):
        ...
        self.V2V_power_dB_List = [23, 15, 5, -100]  # the power levels : [23, 15, 5, -100]
        self.sig2_dB = -114
        self.bsAntGain = 8
        ...
        # self.bandwidth = 1500
        # self.demand_size = int((4 * 190 + 300) * 8 * 50)  # V2V payload: 1060 Bytes every 100 ms
        # self.demand_size = 20
        self.V2V_Rate_threshold = int(1e-5)

        self.V2V_Interference_all = np.zeros((self.n_Veh, self.n_neighbor, self.n_RB)) + self.sig2
        self.j = 0
        # self.V2V_threshold = 5  # 5
        # self.p0 = 0.3 # 0.01
    ...
    def add_new_vehicles_by_number(self, n):
        #####產生目標車輛#####
        start_position = [750 / 2 / 2, 1299 / 2 / 2]  # [200,350]
        choose_direction = ['d','u','l','r']
        j = np.random.randint(0, len(choose_direction)-1)  # 產生0~len(choose_direction)-1的整數
        if j == 0:
            start_direction = 'd'
        elif j == 1:
            start_direction = 'u'
        ...
        # ind = np.random.randint(0, len(self.down_lanes)-1)  # 產生0~len(self.down_lanes)-1的整數
            
        # start_position = [self.down_lanes[ind], self.height / 2]
        # start_direction = 'd' # velocity: 10 ~ 15 m/s, random
        # self.add_new_vehicles(start_position, start_direction, np.random.randint(10, 15))

        
        #####產生鄰近車輛#####
        # start_position = [self.down_lanes[ind], np.random.randint(self.height / 2, self.height / 2 + 10)]
        # start_direction = 'd' # velocity: 10 ~ 15 m/s, random
        # self.add_new_vehicles(start_position, start_direction, np.random.randint(10, 15))
        ...
        # start_position = [self.down_lanes[ind], np.random.randint(self.height / 2 - 10, self.height / 2)]
        # start_direction = 'd' # velocity: 10 ~ 15 m/s, random
        # self.add_new_vehicles(start_position, start_direction, np.random.randint(10, 15))
    ...
    def renew_neighbor(self):
        ...
        for i in range(len(self.vehicles)-1):
            destination = [0]
            self.vehicles[i+1].destinations = destination
    ...
    def renew_channels_fastfading(self):
        ...
        V2V_channels_with_fastfading = np.repeat(self.V2V_channels_abs[:, :, np.newaxis], self.n_RB, axis=2)   # np.repeat（輸入矩陣,元素重複的次數,需要重複的維度）
    ...
    def Compute_Performance_Reward_Train(self, actions_power, i_episode):
        ...
        for i in range(len(self.vehicles)):
            for j in range(self.n_neighbor):
                if not self.active_links[i, j]:  # if not:它測試一個條件是否為假，然後執行一些語句
                    continue
        ...
        V2I_Rate = (self.bandwidth / (1e6)) * np.log2(1 + np.divide(V2I_Signals, self.V2I_Interference))
        ...
        # np.logical_not:這是一個邏輯函數，true會變false，false會變true
        # print("actions:", actions)
        # print("self.active_links:", self.active_links)
        for i in range(self.n_RB):  # scanning all bands
            indexes = np.argwhere(actions == i)  # find spectrum-sharing V2Vs  # np.argwhere(actions == i):返回actions中所有等於i的數字的索引值
        ...
        ###開始
        V2V_SINR = np.divide(V2V_Signal, self.V2V_Interference)
        F_x = np.zeros(shape=len(V2V_SINR), dtype='int32')
        # V2V_eff = self.V2V_threshold/math.log(1/(1-self.p0))
        # V2V_eff = 5
        # A = 50
        # for i in range(len(V2V_SINR)):
        #     if V2V_SINR[i] - V2V_eff >= 0:
        #         F_x[i] = A
        #     else:
        #         F_x[i] = V2V_SINR[i] - V2V_eff
        # V2V_Rate_threshold = 0.2  # V2V payload: 0.2M bits every 1 s
        A = 1
        
        V2V_Rate = (self.bandwidth / (1e6)) * np.log2(1 + np.divide(V2V_Signal, self.V2V_Interference))
        # V2V_Rate = np.log2(1 + np.divide(V2V_Signal, self.V2V_Interference))
        # print("V2V_Signal:", V2V_Signal)
        # print("self.V2V_Interference:", self.V2V_Interference)
        # print("V2V_Rate:", V2V_Rate)
        
        # for i in range(len(V2V_Rate)):
        #     if V2V_Rate[i] - self.V2V_Rate_threshold >= 0:
        #         F_x[i] = A
        #     else:
        #         F_x[i] = (V2V_Rate[i] - self.V2V_Rate_threshold) * 2
        
        # F_x = V2V_Rate - min(V2V_Rate) * np.array([1, 1, 1, 1, 1])
        
        # E_x = np.sum(V2V_Rate)
        
        # G_x = np.sum(V2V_Rate) / self.n_Veh
        # for i in range(len(V2V_Rate)):
        #     if V2V_Rate[i] - np.sum(V2V_Rate) / self.n_Veh >= 0:
        #         F_x[i] = np.sum(V2V_Rate) / self.n_Veh - V2V_Rate[i]
        #     else:
        #         F_x[i] = V2V_Rate[i] - np.sum(V2V_Rate) / self.n_Veh
        
        if i_episode == int(n_episode)/2-1:
            global KK
            KK = min(V2V_Rate)
            # if KK == 0:
            #     KK = np.sort(V2V_Rate)[-4]
            # print("hyper_k:", KK)
        ...
        # if i_episode < 249:
        #     hyper_k = 0
        # elif i_episode > 249 and KK <= min(V2V_Rate):
        #     # global KK
        #     KK = min(V2V_Rate)
        #     hyper_k = KK
        # else:
        ...
        # if (min(V2V_Rate) - hyper_k) > 20:    
        #     F_x = min(V2V_Rate) - hyper_k
        # else:
        #     F_x = (min(V2V_Rate) - hyper_k)*10
        ...
        # if V2V_Rate.all() >= 0.001:   
        #     reward_elements = V2V_Rate / 10
        # elif V2V_Rate.any() >= 0.001:
        #     reward_elements = V2V_Rate / 100
        # else:
        #     reward_elements = 1
        
        ###結束
        
        # self.demand -= V2V_Rate * self.time_fast * self.bandwidth  # c -= a 相當於 c = c - a
        ...
        return V2I_Rate, V2V_Rate, reward_elements, F_x, m, n
    ...
    def Compute_Performance_Reward_Test_rand(self, actions_power):
        ...
        power_selection = actions_power[:, :, 1]  # power selection
        print('2, power_selection: ', power_selection)
        # ------------ Compute V2I rate --------------------
        V2I_Rate = np.zeros(self.n_RB)
        V2I_Interference = np.zeros(self.n_RB)  # V2I interference
        for i in range(len(self.vehicles)):
            for j in range(self.n_neighbor):
                ...
                V2I_Interference[actions[i][j]] += 10 ** ((self.V2V_power_dB_List[power_selection[i, j]] - self.V2I_channels_with_fastfading[i, actions[i, j]]
                                                           + self.vehAntGain + self.bsAntGain - self.bsNoiseFigure) / 10)
                print('3, Selected channel: ', actions[i][j])
                print('4, V2I_Interference of selected channel: ', V2I_Interference[actions[i][j]])
                print('5, Vehicle Antenna Gain: ',self.vehAntGain)
                print('6, Base Station Antenna Gain: ', self.bsAntGain)
                print('7, Base Station Noise Figure: ', self.bsAntGain)
                print('8, V2I channels with fastfading: (not sure)', self.V2I_channels_with_fastfading[i, actions[i, j]])
    ...
    def act_for_training(self, actions, i_episode):

        action_temp = actions.copy()
        V2I_Rate, V2V_Rate, reward_elements, F_x, m, n = self.Compute_Performance_Reward_Train(action_temp, i_episode)
        # print("V2I reward:",np.sum(V2I_Rate) / (self.n_Veh * 10))
        # print("V2V reward:", n / 10)
        # print("V2V reward:",F_x)
        lambdda = 0.2
        # reward = lambdda * np.sum(V2I_Rate) / (self.n_Veh * 10) + (1 - lambdda) * np.sum(F_x)
        # reward = lambdda * np.sum(V2I_Rate) / (self.n_Veh * 10) + (1 - lambdda) * F_x
        reward = lambdda * np.sum(V2I_Rate) / (self.n_Veh * 10) + (1 - lambdda) * n / 10
        reward_c1 = lambdda * np.sum(V2I_Rate) / (self.n_Veh * 10)
        reward_c2 = (1 - lambdda) * n / 10
        # reward = lambdda * np.sum(V2I_Rate) / (self.n_Veh * 10) + (1 - lambdda) * (E_x / (self.n_Veh * self.n_neighbor) - np.sum(F_x))
        # reward = lambdda_a * E_x + lambdda_b * np.sum(F_x) + lambdda_c * G_x
        
        return reward, reward_c1, reward_c2

    def act_for_testing(self, actions):

        action_temp = actions.copy()
        V2I_Rate, V2V_Rate, reward_elements, E_x, F_x = self.Compute_Performance_Reward_Train(action_temp, i_episode)
        count = np.zeros(shape=1, dtype='int32')
        for i in range(len(V2V_Rate)):
            if V2V_Rate[i] - self.V2V_Rate_threshold >= 0:
                count += 1
            else:
        ...
        V2V_success = count / (self.n_Veh * self.n_neighbor)  # V2V success rates

        return V2I_Rate, V2V_success, V2V_Rate, V2V_SINR
