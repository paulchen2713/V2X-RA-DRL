...
% REVISER: Hildo Guillardi J?nior
        ...
        for i = 2:2:nargin 
            ...
            if strcmpi( varargin{i}, 'frozenZoomAspectRatio' )
                if ischar(varargin{i+1}) &&...
                   ( strcmpi(varargin{i+1}, 'on') || strcmpi(varargin{i+1}, 'off') )
                    ...
                    warning('Specified zoom mode not supported. Default values will be applied [%s].', appDataStruct.globalZoomMode);
                end
            end
            if strcmpi( varargin{i}, 'mode' )
                if ischar(varargin{i+1}) &&...
                   ( strcmpi(varargin{i+1}, 'manual') || strcmpi(varargin{i+1}, 'interactive') )
                    ...
                    warning('Specified mode descriptor not supported. Default values will be applied [%s].', appDataStruct.globalMode);
                end
            end
            if strcmpi( varargin{i}, 'magnifierShape' )
                if ischar(varargin{i+1}) &&...
                   ( strcmpi(varargin{i+1}, 'rectangle') || strcmpi(varargin{i+1}, 'ellipse') )
                    ...
                    warning('Specified magnifier shape not supported. Default values will be applied [%s].', appDataStruct.magnifierShape);
                end
            end
                ...
                if ischar(varargin{i+1}) &&...
                   ( strcmpi(varargin{i+1}, 'straight') || strcmpi(varargin{i+1}, 'none') || strcmpi(varargin{i+1}, 'edges') )
                    if ~strcmpi(appDataStruct.magnifierShape, 'rectangle') && strcmpi(varargin{i+1}, 'edges')
                        warning('Specified link style not supported. Default values will be applied for ''displayLinkStyle''[%s].', appDataStruct.linkDisplayStyle);
                    else
                        appDataStruct.linkDisplayStyle = lower(varargin{i+1});
                    end
                else
                    warning('Specified descriptor not supported. Default values will be applied for ''displayLink''[%s].', appDataStruct.linkDisplayStyle);
                end
            end
            if strcmpi( varargin{i}, 'units' )
                if ischar(varargin{i+1}) && strcmpi(varargin{i+1}, 'pixels')
                    ...
                    warning('Specified units descriptor not supported. Default values will be applied [%s].', appDataStruct.globalUnits);
                end
            end
            if strcmpi( varargin{i}, 'edgewidth' )
                if length(varargin{i+1})==1 && isnumeric(varargin{i+1})
                    ...
                    warning('Incorrect edge width value. Default value will be applied [%g].', appDataStruct.globalEdgeWidth );
                end
            end
